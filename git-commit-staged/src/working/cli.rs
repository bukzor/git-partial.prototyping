...
#[derive(Parser, Debug)]
#[command(name = "git-commit-working")]
#[command(about = "Stage and commit working tree changes at specific paths")]
#[command(
    long_about = "Stages paths from working tree, then commits those staged changes.\n\n\
                  ...
                  Examples:\n\
                  \x20 git commit-working src/ -- -m \"Add feature\"\n\
                  \x20 git commit-working src/ tests/ -- --amend\n\
                  \x20 git commit-working . -- --fixup HEAD~1"
)]
pub struct Args {
